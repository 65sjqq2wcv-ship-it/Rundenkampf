class TeamsView {
	...
	render() {
		...
		try {
			...
			// Empty state
			if (storage.teams.length === 0 && storage.standaloneShooters.length === 0) {
			...
			container.appendChild(this.createErrorCard('Fehler beim Laden der Teams-Ansicht'));
		}
	...
	showTeamEditModal(team, isNew = false) {
		...
		const modal = new ModalComponent(isNew ? 'Neue Mannschaft' : 'Mannschaft bearbeiten', content);
		
		modal.addAction('Abbrechen', null, false, false);
		modal.addAction('Speichern', () => {
	...
	setupTeamModalHandlers(team, isNew) {
		...
		if (deleteTeamBtn && !isNew) {
			deleteTeamBtn.addEventListener('click', () => {
				if (confirm(`Möchten Sie die Mannschaft "${team.name}" wirklich löschen?`)) {
	...
	removeShooter(teamId, shooterIndex) {
		...
		if (team && team.shooters[shooterIndex]) {
			...
			if (confirm(`Möchten Sie den Schützen "${shooter.name}" wirklich entfernen?`)) {
	...
	showStandaloneShooterModal(shooter, isNew = false) {
		...
		const modal = new ModalComponent(isNew ? 'Neuer Einzelschütze' : 'Schütze bearbeiten', content);
		...
		setTimeout(() => {
			...
			if (deleteShooterBtn && !isNew) {
				deleteShooterBtn.addEventListener('click', () => {
					if (confirm(`Möchten Sie den Schützen "${shooter.name}" wirklich löschen?`)) {
